...
async function redirectModule(moduleOptions) {
  ...
  let defaults = {
    redirects: {
      headers: {
        common: {
          "X-Client-Type": this.options.redirects?.type || "PC"
        }
      },
      callback: (list = []) => {
        const radioType = { 1: 301, 2: 302 };
        return list && Array.from(list, ({ requestLink, targetLink, type }) => ({ from: decodeURIComponent(requestLink), to: targetLink, code: radioType[type] }))
      }
    },
  }
